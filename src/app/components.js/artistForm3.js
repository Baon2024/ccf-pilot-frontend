...
export default function ArtistForm3({
  ...
  async function sentToBackend(e) {
    ...
    //need to convert to handle stream responses returned
    //will get brandGuide key values from streamed response. 
    ...
    while (true) {
      const { value, done } = await reader.read();
        ...
        if (line.startsWith("data:")) {
          ...
          console.log("initial data from server is: ", json);
          toast("marketing materials being generated!")
          //alert(json);
          setLoading(false)
    setResultsReady(true) 
        } else if (line.startsWith(`pressRelease:`)) {
          console.log("pressRelease generated and returned!!")
          const jsonString = line.replace(`pressRelease:`, '')
          const jsonObj = JSON.parse(jsonString);
          console.log("jsonObj for pressRelease is: ", jsonObj);
          toast.success("press release generated!")
          setPressRelease(jsonObj);
          
        } else if (line.startsWith(`fontAnswer:`)) {
          console.log("fontAnswer generated and returned!!")
          const jsonString = line.replace('fontAnswer:', '')
          const jsonObj = JSON.parse(jsonString);
          console.log("jsonObj for fontAnswer is: ", jsonObj);
          toast.success("font choice and explanations generated!")
          setFontReccomendation({ font: jsonObj.font, explanation: jsonObj.explanation });
        } else if (line.startsWith(`artistLogoOptions:`)) {
          console.log("artistLogoOptions generated and returned!!")
          const jsonString = line.replace(`artistLogoOptions:`, '')
          const jsonObj = JSON.parse(jsonString);
          console.log("jsonObj for artistLogoOptions is: ", jsonObj);
          toast.success("logo options generated!")
          setArtistLogoOptions(jsonObj);
        } else if (line.startsWith(`keyWordsPhrases:`)) {
          console.log("keyWordsPhrases generated and returned!!")
          const jsonString = line.replace(`keyWordsPhrases:`, '')
          const jsonObj = JSON.parse(jsonString);
          console.log("jsonObj for keyWordsPhrases is: ", jsonObj);
          toast.success("Artist keywords and phrases generated!")
          setKeyWordsPhrases(jsonObj);
        }
      }
    }
    toast("marketing materials generated!")
    ...
    console.log("data.brandGuide is: ", data.brandGuide)
    ...
    console.log("data.brandGuide.fontAnswer is: ", data.brandGuide.fontAnswer);
    ...
    setFontReccomendation({ font: data.brandGuide.fontAnswer.font, explanation: data.brandGuide.fontAnswer.explanation })


    const epkFormData = new FormData()
    albumArtFiles?.forEach((file) => {
      epkFormData.append("albumArtFiles", file)
    })
    artistProfilePhotoFiles?.forEach((file) => {
      epkFormData.append("artistProfilePhotoFiles", file)
    })
